...
![主程式介面預覽](screenshots/main_ui.png)
*(▲ 程式執行主畫面)*

這是一個供個人使用的輕量級圖形化小工具，最初只是因為個人工作需求，希望能有一個\*\*在本機端快速執行、介面簡潔、且不需要上傳檔案到雲端\*\*的 PDF 轉圖片工具，本程式能夠將 PDF 檔案快速轉換為圖片格式（PNG 或 JPG），程式介面簡潔，支援拖曳操作與批次轉檔。因為既有工具要嘛功能太複雜、要嘛需要收費或連網，所以決定請 Google Gemini 用 Python 寫一個自己用，現在將它整理出來分享給有相同需求的人。
...
* **加密支援**：自動偵測加密的 PDF 檔案並跳出密碼輸入視窗。
  ![密碼輸入視窗](screenshots/password_dialog.png)
  *(▲ 遇到加密檔案時，會自動彈出解鎖視窗)*
...
如果您不想安裝 Python 環境，只想直接使用軟體，目前我已經將打包成單一執行檔了。
請前往本專案的 **[Releases 頁面](https://github.com/kaoshou/pdf-image-converter/releases)** 下載後直接執行即可，無需安裝任何軟體。
